...
class BacktestingEngine(object):
    ...
    #----------------------------------------------------------------------
    def saveParallelOptimization(self, strategyClass, optimizationSetting):
        """并行优化参数"""
        # 获取优化设置        
        settingList = optimizationSetting.generateSetting()
        targetName = optimizationSetting.optimizeTarget
        ...
        # 检查参数设置问题
        if not settingList or not targetName:
            self.file_output(u'优化设置有问题，请检查')
        ...
        resultList.sort(reverse=True, key=lambda result:result[1])
        self.file_output('-' * 30)
        self.file_output(u'优化结果：')
        for result in resultList:
            self.file_output(u'参数：%s，目标：%s' %(result[0], result[1]))    
        self.file_output('-'*30,end=True)
        
        test_path = os.path.join(os.path.dirname(__file__), "TestLogs")
        if not os.path.exists(test_path):
            os.mkdir(test_path)
            
        strat_path = os.path.join(test_path,self.stratName)
        if not os.path.exists(strat_path):
            os.mkdir(strat_path)
                
        dt = datetime.now().strftime('%Y%m%d_%H%M%S')
        
        txt_name = '{0}_{1}_{2}_optimi.txt'.format(self.stratName, self.symbol, dt)
        txt_file_name = os.path.abspath(os.path.join(strat_path, txt_name))
        f = open(txt_file_name,'w')
        f.write(self.outString.encode('utf-8'))
        f.close()        
        #return resultList        
    #----------------------------------------------------------------------
    ...
    #----------------------------------------------------------------------
    #----------------------------------------------------------------------
    def file_output(self, content, end = False):
        """输出内容"""    
        outstr = str(datetime.now()) + "\t" + content
        self.outList.append(outstr)
        ...
        if end == True:
            ...
            self.outList = []    
    #----------------------------------------------------------------------
    def saveBacktestingResult(self):
        """显示回测结果"""
        d = self.calculateBacktestingResult()
       
        # 输出
        self.file_output('-' * 30)
        self.file_output(u'第一笔交易：\t%s' % d['timeList'][0])
        self.file_output(u'最后一笔交易：\t%s' % d['timeList'][-1])
       
        self.file_output(u'总交易次数：\t%s' % formatNumber(d['totalResult']))       
        self.file_output(u'总盈亏：\t%s' % formatNumber(d['capital']))
        self.file_output(u'最大回撤: \t%s' % formatNumber(min(d['drawdownList'])))               
       
        self.file_output(u'平均每笔盈利：\t%s' %formatNumber(d['capital']/d['totalResult']))
        self.file_output(u'平均每笔滑点：\t%s' %formatNumber(d['totalSlippage']/d['totalResult']))
        self.file_output(u'平均每笔佣金：\t%s' %formatNumber(d['totalCommission']/d['totalResult']))
       
        self.file_output(u'胜率\t\t%s%%' %formatNumber(d['winningRate']))
        self.file_output(u'盈利交易平均值\t%s' %formatNumber(d['averageWinning']))
        self.file_output(u'亏损交易平均值\t%s' %formatNumber(d['averageLosing']))
        self.file_output(u'盈亏比：\t%s' %formatNumber(d['profitLossRatio']))
        self.file_output('-' * 30, end = True)
        ...
        fig_name = '{0}_{1}_{2}_trade.png'.format(self.stratName, self.symbol, dt)
        fig_file_name = os.path.abspath(os.path.join(strat_path, fig_name))
        fig = plt.gcf()
        fig.savefig(fig_file_name)
        print (u'图表每笔交易保存至：{0}'.format(fig_file_name))   
       
        txt_name = '{0}_{1}_{2}_trade.txt'.format(self.stratName, self.symbol, dt)
        txt_file_name = os.path.abspath(os.path.join(strat_path, txt_name))
        f = open(txt_file_name,'w')
        f.write(self.outString.encode('utf-8'))
        f.close()           
    #----------------------------------------------------------------------
    def saveDailyResult(self, df=None, result=None):
        """显示按日统计的交易结果"""
        if df is None:
        ...
        # 输出统计结果
        self.file_output('-' * 30)
        self.file_output(u'首个交易日：\t%s' % result['startDate'])
        self.file_output(u'最后交易日：\t%s' % result['endDate'])
       
        self.file_output(u'总交易日：\t%s' % result['totalDays'])
        self.file_output(u'盈利交易日\t%s' % result['profitDays'])
        self.file_output(u'亏损交易日：\t%s' % result['lossDays'])
       
        self.file_output(u'起始资金：\t%s' % self.capital)
        self.file_output(u'结束资金：\t%s' % formatNumber(result['endBalance']))
   
        self.file_output(u'总收益率：\t%s%%' % formatNumber(result['totalReturn']))
        self.file_output(u'年化收益：\t%s%%' % formatNumber(result['annualizedReturn']))
        self.file_output(u'总盈亏：\t%s' % formatNumber(result['totalNetPnl']))
        self.file_output(u'最大回撤: \t%s' % formatNumber(result['maxDrawdown']))   
        self.file_output(u'百分比最大回撤: %s%%' % formatNumber(result['maxDdPercent']))   
       
        self.file_output(u'总手续费：\t%s' % formatNumber(result['totalCommission']))
        self.file_output(u'总滑点：\t%s' % formatNumber(result['totalSlippage']))
        self.file_output(u'总成交金额：\t%s' % formatNumber(result['totalTurnover']))
        self.file_output(u'总成交笔数：\t%s' % formatNumber(result['totalTradeCount']))
       
        self.file_output(u'日均盈亏：\t%s' % formatNumber(result['dailyNetPnl']))
        self.file_output(u'日均手续费：\t%s' % formatNumber(result['dailyCommission']))
        self.file_output(u'日均滑点：\t%s' % formatNumber(result['dailySlippage']))
        self.file_output(u'日均成交金额：\t%s' % formatNumber(result['dailyTurnover']))
        self.file_output(u'日均成交笔数：\t%s' % formatNumber(result['dailyTradeCount']))
       
        self.file_output(u'日均收益率：\t%s%%' % formatNumber(result['dailyReturn']))
        self.file_output(u'收益标准差：\t%s%%' % formatNumber(result['returnStd']))
        self.file_output(u'Sharpe Ratio：\t%s' % formatNumber(result['sharpeRatio']))
        self.file_output('-' * 30, end=True)
        # 绘图
        fig = plt.figure(figsize=(10, 16))
        ...
        fig_name = '{0}_{1}_{2}_daily.png'.format(self.stratName, self.symbol, dt)
        fig_file_name = os.path.abspath(os.path.join(strat_path, fig_name))
        fig = plt.gcf()
        fig.savefig(fig_file_name)
        print (u'图表每笔交易保存至：{0}'.format(fig_file_name))
       
        txt_name = '{0}_{1}_{2}_daily.txt'.format(self.stratName, self.symbol, dt)
        txt_file_name = os.path.abspath(os.path.join(strat_path, txt_name))
        f = open(txt_file_name,'w')
        f.write(self.outString.encode('utf-8'))
        f.close()

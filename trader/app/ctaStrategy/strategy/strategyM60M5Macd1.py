...
########################################################################
class M60M5MacdStrategy(CtaTemplate):
    """跨时间周期交易策略"""
    className = 'M60M5MacdStrategy'
    author = u'用Python的交易员'

    # 策略参数
    rsiSignal = 20          # RSI信号阈值
    rsiWindow = 14          # RSI窗口
    fastWindow = 12          # 快速均线窗口
    slowWindow = 26       # 慢速均线窗口
    signalWindow = 9
    ...
    def on60MinBar(self, bar):    
        self.am60.updateBar(bar)
        
        if not self.am60.inited:
            return
        
        # 计算均线并判断趋势 diff dea stick
        self.macd,self.signal,self.hist = self.am60.macd(self.fastWindow,self.slowWindow,self.signalWindow)
...
def runBacktesting():
    ...
    bd = {'start':'20110801','capital':10000,'slippage':1,'rate':1.0/10000,'size':10,'priceTick':1,'symbol':'rb0000','stratName':'M60M5MacdStrategy'}
    #bd = {'start':'20150327','capital':200000,'slippage':1,'rate':1.0/10000,'size':1,'priceTick':1,'symbol':'ni0000','stratName':'M60M5MacdStrategy'}
    #bd = {'start':'20140228','capital':200000,'slippage':1,'rate':1.0/10000,'size':5,'priceTick':1,'symbol':'pp0000','stratName':'M60M5MacdStrategy'}
    #bd = {'start':'20080624','capital':200000,'slippage':1,'rate':1.0/10000,'size':5,'priceTick':5,'symbol':'l0000','stratName':'M60M5MacdStrategy'}
    # 设置回测用的数据起始日期
    ...
    # 在引擎中创建策略对象
    d = {}
    engine.initStrategy(M60M5MacdStrategy, d)
...
def runOptimization():
    ...
    engine.saveParallelOptimization(M60M5MacdStrategy, setting)
